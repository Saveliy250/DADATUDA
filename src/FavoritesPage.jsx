...
const FavoriteButtonBlank = ({handleClick}) => {
    return (
        <button className={"favorite-button"} onClick={handleClick}>
            <svg width="26" height="24" viewBox="0 0 26 24" fill="none" xmlns="http://www.w3.org/2000/svg">
...
const LikedCard = ({event, handleClick}) => {
    ...
    return (
        <div className={"liked-card"}>
            <div className={"liked-card-img-wrapper"}>
                ...
                {(event.isFavorite) ? <FavoriteButton handleClick={() => handleClick(event)} /> : <FavoriteButtonBlank handleClick={() => handleClick(event)} />}
                <TrashButton />
...
function Favorites() {
    ...
    const starredEvents = favorites.filter(event => event.isFavorite === true);
    const notStarredEvents = favorites.filter(event => !event.isFavorite);
        ...
        <>
            ...
            {favorites.length > 0 && (<div className={"liked-cards"}>
                {starredEvents.map((event) => (
                    <LikedCard key={event.id} event={event} handleClick={() => onFavoriteClick(event)} />
                ))}
            </div>)}
